...
module.exports = Router()
  ...
  .put('/api/v1/bicycles/:id', async (req, res) => {
    try {
      const bicycle = await Bicycle.update(req.body);
      res.send(bicycle);
    } catch(err) {
      res.status(500). send({ error: err.message });
    }
  ...
  .delete('/api/v1/bicycles/:id', async (req, res) => {
    try {
      const bicycle = await Bicycle.delete(req.params.id);
      res.send(bicycle);
    } catch(err) {
      res.status(500).send({ error: err.message });
    }
